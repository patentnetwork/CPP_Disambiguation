...
public:
	...
	~cCluster();
	//pay attention to the copy constructor (NOT const reference )
	...
	unsigned int reset_coauthor_list(const map < const cRecord *, cGroup_Value, cSort_by_attrib > & reference_patent_tree);
	...
	const set < const string * > & get_coauthor_set() const {return coauthor_list;}
